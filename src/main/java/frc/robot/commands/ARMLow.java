...
public class ARMLow extends ParallelCommandGroup {
  ...
  public ARMLow(SWrist m_sWrist, SExtendElevator m_sExtendElevator, SLiftArm m_sLiftArm, SGripper m_sGripper) {
    // Add your commands in the addCommands() call, e.g.
    // addCommands(new FooCommand(), new BarCommand());
    addCommands(
      new LiftArm(m_sLiftArm, 10).withInterruptBehavior(InterruptionBehavior.kCancelSelf),
      new ExtendElevator(m_sExtendElevator, 0).withInterruptBehavior(InterruptionBehavior.kCancelSelf),
      new Wrist(m_sWrist, -11).withInterruptBehavior(InterruptionBehavior.kCancelSelf)
    );
  }
}
