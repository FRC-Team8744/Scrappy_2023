...
// private final ChangeLED toggleLed = new ChangeLED(m_lightbar);
  ...
  */
  ...
  private RobotContainer() {
    ...
    m_chooser.setDefaultOption("Autonomous Command", new AutonomousCommand( m_drivetrain, 144.0));//inches
...
final POVButton wristClose = new POVButton(joystick1, 180); 
wristClose.onTrue(new Wrist(20, m_sWrist).withInterruptBehavior(InterruptionBehavior.kCancelSelf));

                        
final POVButton elevatorExtentionMid = new POVButton(gamePad, 90);        
elevatorExtentionMid.onTrue(new ExtendElevator(-65, m_sExtendElevator).withInterruptBehavior(InterruptionBehavior.kCancelSelf));
...
gripperClose.whileTrue(new Gripper(0, m_sGripper).withInterruptBehavior(InterruptionBehavior.kCancelSelf));

// final JoystickButton wristOpen = new JoystickButton(gamePad, 4);    
final POVButton wristOpen = new POVButton(joystick1, 90); 
wristOpen.onTrue(new Wrist(30, m_sWrist).withInterruptBehavior(InterruptionBehavior.kCancelSelf));
                        // SmartDashboard.putData("WristOpen",new Wrist(10, m_sWrist));

final POVButton wristUp = new POVButton(joystick1, 0); 
wristUp.onTrue(new Wrist(40, m_sWrist).withInterruptBehavior(InterruptionBehavior.kCancelSelf));
                        // SmartDashboard.putData("WristOpen",new Wrist(10, m_sWrist));
                        
final POVButton extendElevatorHigh = new POVButton(gamePad, 0);        
extendElevatorHigh.onTrue(new ExtendElevator(-110, m_sExtendElevator).withInterruptBehavior(InterruptionBehavior.kCancelSelf));
                        // SmartDashboard.putData("ExtendElevatorHigh",new ExtendElevator(10, m_sExtendElevator));
   
final POVButton extendElevatorLow = new POVButton(gamePad, 180);        
extendElevatorLow.onTrue(new ExtendElevator(-2, m_sExtendElevator).withInterruptBehavior(InterruptionBehavior.kCancelSelf));
                        // SmartDashboard.putData("ExtendElevatorHigh",new ExtendElevator(10, m_sExtendElevator));
...
liftArmHigh.onTrue(new LiftArm(10, m_sLiftArm).withInterruptBehavior(InterruptionBehavior.kCancelSelf));
 ...
 liftArmMid.onTrue(new LiftArm(10, m_sLiftArm).withInterruptBehavior(InterruptionBehavior.kCancelSelf));
                        
final JoystickButton gripperOpen = new JoystickButton(joystick1, 3); //gamePad, 1);        
gripperOpen.whileTrue(new Gripper(10, m_sGripper).withInterruptBehavior(InterruptionBehavior.kCancelSelf));
