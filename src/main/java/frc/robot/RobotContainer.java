...
// private final ChangeLED toggleLed = new ChangeLED(m_lightbar);
  ...
  */
  ...
  private RobotContainer() {
    ...
    m_chooser.addOption("Auto Middle Position", new AutoDrive(m_drivetrain, m_sWrist, m_sExtendElevator, m_sLiftArm, m_sGripper));
    m_chooser.addOption("Auto Short Community", new AutonomousCommand( m_drivetrain, 48.0));
    m_chooser.addOption("Auto Long Community", new AutonomousCommand( m_drivetrain, 72.0));
...
liftArmHigh.onTrue(new LiftArm(m_sLiftArm, 10 ).withInterruptBehavior(InterruptionBehavior.kCancelSelf));
                        // SmartDashboard.putData("LiftArmHigh",new LiftArm(10, m_sLiftArm));

 final JoystickButton liftArmMid = new JoystickButton(gamePad, 2);        
 liftArmMid.onTrue(new LiftArm(m_sLiftArm, 10).withInterruptBehavior(InterruptionBehavior.kCancelSelf));
